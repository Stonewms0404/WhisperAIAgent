...
static int process_general_transcription(struct whisper_context * ctx, audio_async & audio, const whisper_params & params, std::ofstream & fout, Parser* parser) {
    bool is_running  = true;
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy)) {
                ...
                if (!have_prompt) {
                    ...
                    pcmf32_cur.insert(pcmf32_cur.begin(), 2.0f*WHISPER_SAMPLE_RATE, 0.0f);
                    ...
                    if (best_len == 0) {
                        ...
                        if (parser->parse_command(command)) {
                            fprintf(stdout, "%s: Command '%s%s%s', Success, (t = %d ms)\n", __func__, "\033[1m", command.c_str(), "\033[0m", (int)t_ms);
                        }
                        else {
                            fprintf(stdout, "%s: Command '%s%s%s', Failed, (t = %d ms)\n", __func__, "\033[1m", command.c_str(), "\033[0m", (int)t_ms);
                        }
                        if (fout.is_open()) {
                            fout << command << std::endl;
                        }
...
int Command::listen_to_input(int argc, char ** argv) {
    printf("\nListening to input...\n");
    ...
    if (ret_val == 0) {
        ...
        ret_val = process_general_transcription(ctx, audio, params, fout, m_parser);
    }
